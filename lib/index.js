// dsh-ui-appearance — host half (minimal).
//
// Responsibilities:
//   * persist plugin config (wallpaper ref, opacity/blur, glass, accent)
//     as a small JSON document under $DSH_HOME/ui-appearance/
//   * accept background image uploads and store them under
//     $DSH_HOME/ui-appearance/backgrounds/, returning a server-relative URL
...
const DATA_SUBDIR = 'ui-appearance'
...
export default {
  ...
  apply(ctx) {
    ...
    // GET/POST /ui-appearance/config
    webServer.register({
      kind: 'exact',
      path: '/ui-appearance/config',
    ...
    // GET /ui-appearance/state — authoritative native theme preference
    webServer.register({
      kind: 'exact',
      path: '/ui-appearance/state',
      handler: async (_req, res) => {
        sendJson(res, { ok: true, nativePreference: nativePreference() })
      }
    })

    // POST /ui-appearance/background — upload an image (raw body)
    webServer.register({
      kind: 'exact',
      path: '/ui-appearance/background',
      handler: async (req, res) => {
        try {
          ...
          sendJson(res, { ok: true, url: `/ui-appearance/backgrounds/${id}${ext}` })
        } catch (e) {
          sendJson(res, { ok: false, error: e instanceof Error ? e.message : String(e) })
        }
      }
    ...
    // GET /ui-appearance/backgrounds/* — serve stored background images.
    ...
    webServer.register({
      kind: 'prefix',
      path: '/ui-appearance/backgrounds',
      handler: async (req, res) => {
        try {

// dsh-ui-appearance — web client half (minimal, no build step).
//
...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    // ---- injected stylesheet (namespaced with --dsh-appearance-*) ----
    ...
    var CSS = [
      ':root{--dsh-appearance-bg-image:none;--dsh-appearance-bg-opacity:.35;--dsh-appearance-bg-blur:0px}',
      // Background layer: fixed, behind the app frame (before #root in DOM),
      // non-interactive. Bleed by 40px so the blur edge never shows.
      '.dsh-ui-appearance-bg{position:fixed;inset:-40px;z-index:0;pointer-events:none;background-image:var(--dsh-appearance-bg-image);background-size:cover;background-position:center;filter:blur(var(--dsh-appearance-bg-blur));opacity:var(--dsh-appearance-bg-opacity)}'
    ].join('\n')
    ...
    function getConfig() { return api('/ui-appearance/config', { method: 'GET', headers: { Accept: 'application/json' } }) }
    function saveConfig(c) {
      return api('/ui-appearance/config', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(c) })
    }
    ...
    function applyBackground(c) {
      ...
      root.style.setProperty('--dsh-appearance-bg-image', on && c.backgroundImage ? cssUrl(c.backgroundImage) : 'none')
      root.style.setProperty('--dsh-appearance-bg-opacity', String(finite(c.backgroundOpacity, 0.35)))
      root.style.setProperty('--dsh-appearance-bg-blur', String(finite(c.backgroundBlur, 0)) + 'px')
      if (on) {
        if (!bgEl) {
          bgEl = document.createElement('div')
          bgEl.className = 'dsh-ui-appearance-bg'
          bgEl.setAttribute('data-dsh-ui-appearance', 'bg')
    ...
    function applyThemeLayer(c) {
      ...
      try { theme.overrideTokens('dsh-ui-appearance', buildTokens(c)) } catch (e) { /* non-fatal */ }
    }
    ...
    function SettingsPanel() {
      ...
      function onUpload(ev) {
        ...
        if (!file) return
        api('/ui-appearance/background', { method: 'POST', headers: { 'Content-Type': file.type || 'image/jpeg' }, body: file })
          .then(function (r) {
    ...
    function apply(ctx) {
      ...
      try {
        ctx.effect(function () {
          ...
          tag.setAttribute('data-plugin', 'dsh-ui-appearance')
          tag.setAttribute('data-plugin-css', 'dsh-ui-appearance/base')
          tag.textContent = CSS
          document.head.appendChild(tag)
          return function () { tag.remove() }
        }, 'dsh-ui-appearance: base stylesheet')
      } catch (e) { /* non-fatal */ }
      ...
      try {
        ...
        }, 'dsh-ui-appearance: theme/change listener')
      } catch (e) { /* non-fatal */ }
        ...
        try {
          slots.inject('settings.section', function () {
            return slots.register(
              { name: 'settings.section', id: 'dsh-ui-appearance', order: 50, label: '外观 (Appearance)' },

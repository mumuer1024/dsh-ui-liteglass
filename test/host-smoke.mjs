// dsh-ui-appearance — host-side end-to-end smoke test.
//
...
const tmpHome = await mkdtemp(join(tmpdir(), 'dsh-ui-appearance-test-'))
...
try {
  ...
  console.log('\n== GET /ui-appearance/config (fresh) ==')
  let res = await webServer.dispatch('GET', '/ui-appearance/config', {})
  let j = JSON.parse(res.body)
  check('GET config → 200', res.statusCode === 200)
  check('GET config ok', j.ok === true)
  check('GET config null on fresh', j.config === null)

  console.log('\n== POST /ui-appearance/config (valid) ==')
  res = await webServer.dispatch('POST', '/ui-appearance/config', { 'Content-Type': 'application/json' }, Buffer.from(JSON.stringify({
  ...
  res = await webServer.dispatch('GET', '/ui-appearance/config', {})
  j = JSON.parse(res.body)
  ...
  res = await webServer.dispatch('POST', '/ui-appearance/config', { 'Content-Type': 'application/json' }, Buffer.from(JSON.stringify({ background: 'local' })))
  j = JSON.parse(res.body)
  const c = j.config
  ...
  res = await webServer.dispatch('POST', '/ui-appearance/config', { 'Content-Type': 'application/json' }, Buffer.from('not json'))
  j = JSON.parse(res.body)
  ...
  console.log('\n== GET /ui-appearance/state (nativePreference from settings) ==')
  res = await webServer.dispatch('GET', '/ui-appearance/state', {})
  j = JSON.parse(res.body)
  ...
  res = await webServer2.dispatch('GET', '/ui-appearance/state', {})
  j = JSON.parse(res.body)
  ...
  res = await webServer3.dispatch('GET', '/ui-appearance/state', {})
  j = JSON.parse(res.body)
  check('no-settings nativePreference = system', j.ok === true && j.nativePreference === 'system')

  console.log('\n== POST /ui-appearance/background (valid PNG) ==')
  const pngBytes = Buffer.from('89504e470d0a1a0a0000000d49484452', 'hex')
  res = await webServer.dispatch('POST', '/ui-appearance/background', { 'Content-Type': 'image/png' }, pngBytes)
  j = JSON.parse(res.body)
  check('upload ok', j.ok === true)
  check('upload url shape', /^\/ui-appearance\/backgrounds\/[0-9a-f]{16}\.png$/.test(j.url))
  const bgUrl = j.url
  const bgId = bgUrl.split('/').pop()

  console.log('\n== GET /ui-appearance/backgrounds/<id> (serve round-trip) ==')
  ...
  console.log('\n== on-disk persistence ==')
  const configFile = join(tmpHome, 'ui-appearance', 'config.json')
  const bgFile = join(tmpHome, 'ui-appearance', 'backgrounds', bgId)
  let cfgOnDisk = null
  ...
  res = await webServer.dispatch('POST', '/ui-appearance/background', { 'Content-Type': 'text/html' }, Buffer.from('<html></html>'))
  j = JSON.parse(res.body)
  check('unsupported mime rejected', j.ok === false && /unsupported content type/.test(j.error))

  console.log('\n== path traversal / bad names rejected (400) ==')
  const badNames = [
    '/ui-appearance/backgrounds/../../etc/passwd',
    '/ui-appearance/backgrounds/..',
    '/ui-appearance/backgrounds/.',
    '/ui-appearance/backgrounds/x.png',
    '/ui-appearance/backgrounds/nothexatall.png',
    '/ui-appearance/backgrounds/../../secret'
  ]
  ...
  res = await webServer.dispatch('GET', '/ui-appearance/backgrounds/' + '0'.repeat(16) + '.png', {})
  ...
  const outsideProbe = join(tmpdir(), 'ui-appearance-outside-marker')
